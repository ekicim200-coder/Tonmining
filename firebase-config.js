...
// ========================================
// NOTIFICATION SYSTEM - Firebase Integration
// ========================================
...
async function saveNotificationPreferences(userId, preferences) {
  try {
    await setDoc(doc(db, "users", userId), {
      notifications: {
        telegram: {
          enabled: preferences.telegramEnabled || false,
          chatId: preferences.telegramChatId || null,
          types: {
            mining: preferences.telegramMining !== false,
            rewards: preferences.telegramRewards !== false,
            withdrawals: preferences.telegramWithdrawals !== false,
            referrals: preferences.telegramReferrals !== false,
            system: preferences.telegramSystem !== false
          }
        },
        browser: {
          enabled: preferences.browserEnabled || false,
          types: {
            daily: preferences.browserDaily !== false,
            system: preferences.browserSystem !== false,
            rewards: preferences.browserRewards !== false
          }
        },
...
// Belirli bildirim tipini almak isteyen kullanıcıları getir
async function getUsersForNotification(notificationType) {
  try {
    const usersRef = collection(db, "users");
    const q = query(
      usersRef,
      where(`notifications.telegram.enabled`, "==", true),
      where(`notifications.telegram.types.${notificationType}`, "==", true)
    );
    
    const snapshot = await getDocs(q);
    return snapshot.docs.map(doc => ({
      userId: doc.id,
      chatId: doc.data().notifications?.telegram?.chatId,
      ...doc.data()
    }));
  } catch (error) {
...
async function saveNotificationHistory(userId, notification) {
  try {
    ...
    await addDoc(historyRef, {
      ...
      channel: notification.channel, // 'telegram' or 'browser'
      sentAt: Date.now(),
      read: false
    });
    return { success: true };

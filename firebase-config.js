...
// KYC Onaylama Fonksiyonu
export async function updateKYCStatus(withdrawalId, kycVerified) {
    if (!currentUser) {
        console.error("❌ Hata: Kullanıcı giriş yapmamış!");
        return false;
    }
    ...
    try {
        ...
        await setDoc(withdrawalRef, {
            ...
            kycVerifiedDate: kycVerified ? Date.now() : null
        }, { merge: true });
        
        console.log(`✅ KYC durumu güncellendi: ${withdrawalId} - Verified: ${kycVerified}`);
        return true;
    } catch (error) {
        console.error("❌ KYC güncelleme hatası:", error.code, error.message);
        return false;
    }
}

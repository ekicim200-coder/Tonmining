// app.js
// 1. Config dosyasından 'where' dahil her şeyi alıyoruz
...
// --- 7. ÇEKİM VE GEÇMİŞ (AYRI KLASÖR MANTIĞI) ---
async function processWithdraw() {
    ...
    try {
        // 'withdrawals' ana klasörüne kaydediyoruz
        const withdrawRef = collection(db, "withdrawals");
        await addDoc(withdrawRef, {
            userId: userID, // Kimin çektiğini yazıyoruz
            amount: val,
            addr: walletAddr,
            status: "Pending",
...
async function fetchAndRenderHistory() {
    ...
    try {
        ...
        // Sadece BU kullanıcının verilerini çek (where userId == userID)
        const q = query(
            historyRef, 
            where("userId", "==", userID), 
        ...
        if(error.message.includes("index")) {
            console.log("⚠️ LÜTFEN KONSOLDAKİ LİNKE TIKLAYIP INDEX OLUŞTURUN!");
            list.innerHTML = '<div class="text-center text-yellow-500 text-xs py-4">Setup Index via Console</div>';
        }
    }
}
...
function addMachine(id) { const p = products.find(x => x.id === id); if(!gameState.inventory[id]) gameState.inventory[id] = 0; gameState.inventory[id]++; if(!gameState.mining) { gameState.mining = true; activateSystem(); } showToast(`Purchased ${p.name}!`, 'success'); recalcStats(); updateUI(); renderMarket(); saveGame(true); }
function renderMarket() { const list = document.getElementById('market-list'); if(!list) return; list.innerHTML = ''; products.forEach(p => { const count = gameState.inventory[p.id] || 0; const dailyInc = (p.income * 86400).toFixed(2); const div = document.createElement('div'); div.className = "glass-panel p-5 rounded-2xl flex flex-col justify-between transition border border-gray-800 hover:border-cyan-500/50"; div.innerHTML = `<div class="flex justify-between items-start mb-4"><div class="p-4 bg-black/40 rounded-xl ${p.color} text-2xl shadow-inner border border-white/5"><i class="fa-solid ${p.icon}"></i></div><div class="text-xs bg-gray-900 px-3 py-1 rounded-full text-gray-400 border border-gray-700">Owned: ${count}</div></div><div class="mb-4"><h3 class="font-bold text-lg text-white">${p.name}</h3><div class="flex items-center gap-3 mt-2 text-xs"><span class="text-gray-400"><i class="fa-solid fa-bolt text-yellow-500"></i> ${p.hash} TH/s</span></div><div class="text-xs text-green-400 mt-1 font-bold">+${dailyInc} TON / Day</div></div><div class="flex gap-2"><button onclick="window.gameApp.buyWithTON(${p.id})" class="btn-ton-check w-1/2 btn-ton py-3 rounded-xl font-bold text-xs flex justify-center items-center gap-1" data-price="${p.priceTON}">${p.priceTON} TON</button><button onclick="window.gameApp.buyWithStars(${p.id})" class="w-1/2 btn-star py-3 rounded-xl font-bold text-xs flex justify-center items-center gap-1">${p.priceStars} <i class="fa-solid fa-star text-white"></i></button></div>`; list.appendChild(div); }); updateUI(); }
function updateUI() { const b = gameState.balance.toFixed(7); if(document.getElementById('main-balance')) document.getElementById('main-balance').innerText = b + " TON"; if(document.getElementById('mobile-balance')) document.getElementById('mobile-balance').innerText = b; if(document.getElementById('wallet-balance-display')) document.getElementById('wallet-balance-display').innerText = b; if(document.getElementById('dash-hash')) document.getElementById('dash-hash').innerText = gameState.hashrate.toLocaleString(); if(document.getElementById('dash-daily')) document.getElementById('dash-daily').innerText = (gameState.income * 86400).toFixed(2); if(document.getElementById('dash-income')) document.getElementById('dash-income').innerText = gameState.income.toFixed(7); if(document.getElementById('dash-devices')) document.getElementById('dash-devices').innerText = Object.values(gameState.inventory).reduce((a,b)=>a+b,0); document.querySelectorAll('.btn-ton-check').forEach(btn => { const price = parseFloat(btn.getAttribute('data-price')); btn.disabled = gameState.balance < price; }); }
function renderInventory() { const list = document.getElementById('inventory-list'); if(!list) return; list.innerHTML = ''; let empty = true; products.forEach(p => { const count = gameState.inventory[p.id] || 0; if(count > 0) { empty = false; const div = document.createElement('div'); div.className = "glass-panel p-4 rounded-xl flex items-center gap-4 border-l-4 border-cyan-500 bg-gradient-to-r from-cyan-900/10 to-transparent"; div.innerHTML = `<div class="w-12 h-12 bg-black/40 rounded-lg flex items-center justify-center ${p.color} text-2xl"><i class="fa-solid ${p.icon}"></i></div><div class="flex-1"><h4 class="font-bold text-white">${p.name}</h4><div class="text-xs text-gray-400">Total: <span class="text-white">${(p.hash * count).toLocaleString()} TH/s</span></div></div><div class="text-xl font-bold digit-font text-white bg-white/5 px-3 py-1 rounded-lg">x${count}</div>`; list.appendChild(div); } }); if(empty) list.innerHTML = '<div class="col-span-2 text-center text-gray-500 py-10 italic">Warehouse empty.</div>'; }

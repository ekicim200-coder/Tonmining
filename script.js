...
// --- REFERRAL FROM MY DEVICES PAGE ---
window.applyReferralFromInv = async function() {
    const input = document.getElementById('invRefInput');
    const msg   = document.getElementById('invRefMsg');
    if (!input || !msg) return;
    ...
    const showMsg = (text, success) => {
        ...
        msg.innerHTML     = `<i class="fas fa-${success ? 'check-circle' : 'times-circle'}"></i> ${text}`;
    };

    if (!code) { showMsg('Please enter a code!', false); return; }
    if (!code.startsWith('REF') || code.length < 8) { showMsg('Invalid code format!', false); return; }

    // Zaten kullanılmış mı?
    if (state.referredBy || state.referralLocked) {
        showMsg('You have already used a referral code!', false);
        return;
    }

    // Cüzdan bağlı değilse kaydet, bağlandığında uygula
    if (!state.wallet) {
        referralCodePending = code;
        localStorage.setItem('hasUsedReferral', 'applied');
        showMsg('Code saved! Connect your wallet to activate.', true);
        // Popup'taki input'u da doldur
        const popupInput = document.getElementById('refInput');
        if (popupInput) popupInput.value = code;
        return;
    }

    // Cüzdan bağlıysa hemen uygula
    showMsg('Activating...', true);
    const success = await registerReferral(state.wallet, code);
    if (success) {
        ...
        if (!state.referralBonusReceived) {
            await giveReferralBonus(state.wallet);
        }
        saveLocalData();
        showMsg('✅ Referral code applied! Bonus received.', true);
        input.disabled = true;
    } else {
        showMsg('Invalid or already used code.', false);
    }
};

// --- REFERRAL FROM REFERRAL PAGE ---
window.applyReferralFromPage = async function() {
    const input = document.getElementById('enterRefInput');
    ...
    if (success) {
        ...
        localStorage.setItem('hasUsedReferral', 'applied');
        referralCodePending = null;
        if (!state.referralBonusReceived) await giveReferralBonus(state.wallet);
        saveLocalData();
        showMsg('✅ Referral code applied! Bonus received.', true);
...
window.copyRefLink = function() {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(refLinkInput.value).then(() => showToast('✅ Link copied!', false));
    } else {
        refLinkInput.select();
...
// Cüzdan bağlandıktan sonra invRef bölümünü güncelle
function updateInvRefSection() {
    const section = document.getElementById('invRefCodeSection');
    const input   = document.getElementById('invRefInput');
    const msg     = document.getElementById('invRefMsg');
    if (!section) return;

    if (state.referredBy || state.referralLocked) {
        // Kod zaten kullanılmış — bölümü kapat/gizle
...
async function renderHistory() {
    ...
    history.forEach(tx => {
        ...
        // KYC durumu kontrolü
        const kycStatus = tx.kycVerified || false;
        const showKycButton = tx.status === 'pending' && !kycStatus;

        html += `
        <div style="background: rgba(255,255,255,0.05); padding: 12px; margin-bottom: 10px; border-radius: 8px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                <div>
                    <div style="font-weight: bold; color: #fff; font-size: 1rem;">-${tx.amount} TON</div>
                    <div style="font-size: 0.75rem; color: #888;">${date}</div>
                </div>
                <div style="text-align: right;">
                    <div style="color: ${color}; font-size: 0.85rem; font-weight: bold; margin-bottom: 4px;">
                        ${statusText}
                    </div>
                    ${kycStatus ? '<div style="font-size: 0.7rem; color: #10b981;">✓ KYC Verified</div>' : ''}
                </div>
            </div>
            ${showKycButton ? `
            <button 
                onclick="openKYC('${tx.id || ''}')" 
                style="width: 100%; padding: 10px; background: linear-gradient(135deg, #3b82f6, #2563eb); color: white; border: none; border-radius: 8px; font-weight: bold; cursor: pointer; font-size: 0.9rem; display: flex; align-items: center; justify-content: center; gap: 8px;"
...
function updateReferralUI() {
    ...
    const enterRefSection = document.getElementById('enterRefSection');
    const enterRefInput   = document.getElementById('enterRefInput');

    // Kendi kodu — büyük görünür display + gizli input (copy için)
    if (state.referralCode) {
        if (refCodeDisplay) refCodeDisplay.textContent = state.referralCode;
        if (refCodeInput)   refCodeInput.value         = state.referralCode;
        if (refLinkInput)   refLinkInput.value         = `https://t.me/${BOT_USERNAME}?start=${state.referralCode}`;
    }

    if (refCountEl)    refCountEl.textContent    = state.referralCount || 0;
    if (refEarningsEl) refEarningsEl.textContent = (state.referralEarnings || 0).toFixed(2);

    // Başkasının kodunu kullandıysa
    if (state.referredBy || state.referralLocked) {
        if (refStatusBadge) refStatusBadge.style.display = 'block';
        if (refByCode)      refByCode.textContent = (state.referredBy || '').substring(0, 12) + (state.referredBy && state.referredBy.length > 12 ? '...' : '');
        if (enterRefSection) enterRefSection.style.display = 'none'; // kod kullanıldıysa gizle
    } else {
        if (refStatusBadge)  refStatusBadge.style.display  = 'none';
        if (enterRefSection) enterRefSection.style.display = 'block';
        if (enterRefInput)   enterRefInput.disabled         = false;
    }
}
...
function copyReferralCode() {
    ...
    const toCopy = state.referralCode;
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(toCopy).then(() => {
            ...
            const refCodeInput = document.getElementById('ref-code');
            if (refCodeInput) { refCodeInput.select(); document.execCommand('copy'); }
            showToast("✅ Code copied!", false);
        });
    } else {
        const refCodeInput = document.getElementById('ref-code');
...
// KYC Verification fonksiyonu
window.openKYC = async function(withdrawalId) {
    if (!state.wallet) {
        showToast("Connect wallet first!", true);
        return;
    }
    
    // SEÇENEK 1: Ayrı KYC Botu kullan (önerilir)
    const KYC_BOT_USERNAME = "YourKYCBot"; // ⚠️ KYC botunuz varsa buraya yazın
    
    // SEÇENEK 2: Destek botunu kullan (geçici çözüm)
    const SUPPORT_USERNAME = "YourSupportBot"; // ⚠️ Destek bot/kanal username
    
    // SEÇENEK 3: Ana botu kullan
    const MAIN_BOT_USERNAME = BOT_USERNAME;
    
    // Hangi bot kullanılacak (öncelik sırasına göre)
    let botUsername = KYC_BOT_USERNAME;
    
    // Eğer KYC bot ayarlanmamışsa, destek botunu kullan
    if (botUsername === "YourKYCBot") {
        botUsername = SUPPORT_USERNAME;
        
        // Eğer destek bot da yoksa, ana botu kullan
        if (botUsername === "YourSupportBot") {
            botUsername = MAIN_BOT_USERNAME;
        }
    }
    
    // KYC mesajı hazırla
    const kycMessage = encodeURIComponent(
        `🔐 KYC VERIFICATION REQUEST\n\n` +
        `Wallet: ${state.wallet}\n` +
        `Withdrawal ID: ${withdrawalId}\n` +
        `Amount: Check withdrawal history\n\n` +
        `Please verify my identity to process withdrawal.`
    );
    
    // URL oluştur
    let kycUrl;
    
    if (botUsername.startsWith('@') || botUsername.length < 5) {
        // Kanal veya geçersiz username ise web.telegram açalım
        kycUrl = `https://t.me/${botUsername.replace('@', '')}`;
    } else {
        // Bot ise start parametresi ile aç
        kycUrl = `https://t.me/${botUsername}?start=kyc_${withdrawalId}&text=${kycMessage}`;
    }
    ...
    if (tg) {
        try {
            // Telegram içindeyse direkt aç
            tg.openTelegramLink(kycUrl);
            if (tg.HapticFeedback) {
                tg.HapticFeedback.impactOccurred('medium');
            }
            showToast("✅ Opening KYC...", false);
        } catch (error) {
            console.error('Telegram link error:', error);
            // Hata olursa normal link aç
            window.open(kycUrl, '_blank');
            showToast("KYC window opened", false);
        }
    } else {
        // Telegram dışındaysa yeni sekmede aç
        window.open(kycUrl, '_blank');
        showToast("KYC window opened", false);
    }
    ...
    setTimeout(() => {
        showToast(`📱 Send verification docs to @${botUsername}`, false);
    }, 2000);
}

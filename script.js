    ...
    try {
        ...
        if (serverData) {
            ...
            if (serverData.claimedTasks && serverData.claimedTasks.length > 0) {
                const localClaimed = getClaimedTasks();
                const merged = [...new Set([...localClaimed, ...serverData.claimedTasks])];
                saveClaimedTasks(merged);
            }
...
function checkFree() {
    if (!checkFree._el) {
        checkFree._el = {
            ad: document.getElementById('adBtnArea'),
            timer: document.getElementById('timerArea'),
            text: document.getElementById('freeTimer')
        };
    }
    const el = checkFree._el;
    
    if (!el.ad || !el.timer || !el.text) return;
    ...
    if (state.freeEnd > 0 && Date.now() < state.freeEnd) {
        ...
        const s = left % 60;
        el.text.textContent = `${m}:${s.toString().padStart(2,'0')}`;
    } else {
        if (state.freeEnd > 0) {
...
function checkPromoMachines() {
    ...
    let hashLost = 0;
    expired.forEach(item => { hashLost += machineRates[item.mid] || 0; });
    ...
    saveLocalData();
    // Reset integrity baseline so sync doesn't flag balance drop as suspicious
    localStorage.setItem('lastSyncedBalance', state.balance.toString());
    syncToServer(); // Uses normal 30s throttle
    updateUI();
    drawChart();
    renderInv();
    
    showToast(`⏰ ${expired.length} promo machine(s) expired (-${hashLost} GH/s)`, true);
}
        ...
        tg.openInvoice(invoiceLink, async (status) => {
            if (status === 'paid') {
                ...
                try {
                    ...
                    console.error("Validate purchase error:", e);
...
function graphLoop() {
    if (!graphLoop._el) {
        graphLoop._el = {
            card: document.getElementById('liveGraphCard'),
            line: document.getElementById('liveLine'),
            fill: document.getElementById('liveFill'),
            dot: document.getElementById('liveDot'),
            status: document.getElementById('networkStatus'),
            stats: document.getElementById('lgStats'),
            hash: document.getElementById('lgHash')
        };
    }
    const el = graphLoop._el;

    if (!el.line || !el.card) return;

    const isActive = state.hashrate > 0;

    if (isActive) {
        el.card.classList.add('active');
        if (el.status) el.status.innerHTML = `<span style="color:var(--success)">${t('mining')}</span>`;
        if (el.stats) el.stats.style.display = 'flex';
        if (el.hash) el.hash.textContent = state.hashrate;
    } else {
        el.card.classList.remove('active');
        if (el.status) el.status.innerHTML = `<span style="color:var(--text-muted)">${t('offline')}</span>`;
    ...
    if (el.fill) {
        ...
        el.fill.setAttribute('points', fillPoints);
    }

    // Move dot to last point
    if (el.dot) {
        const lastVal = graphData[graphData.length - 1];
        el.dot.setAttribute('cy', (25 - lastVal).toFixed(1));
    }
}
...
function termLoop() {
    if (!termLoop._el) termLoop._el = document.getElementById('termLogs');
    const term = termLoop._el;
    if (!term) return;
    ...
    try {
        ...
        if (data.success) {
            if (data.rewardType === 'machine') {
                ...
                // Calculate promoExpiry CLIENT-SIDE using machineDuration from response
                const durationMin = data.machineDuration || 30;
                const expiry = Date.now() + (durationMin * 60000);
                console.log('🎁 Promo machine: ' + durationMin + ' min, expires: ' + new Date(expiry).toLocaleTimeString());
                state.inv.push({
...
window.claimTask = async function(taskId) {
    ...
    if (state.wallet && currentUserUid) {
        ...
        try {
            showToast("Claiming...", false);
            const response = await fetch('/api/claim-task', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ walletAddress: state.wallet, userId: currentUserUid, taskId })
            });
            const data = await response.json();
            
            if (data.success) {
                state.balance = data.newBalance;
                // Sync claimed tasks from server
                claimed.push(taskId);
                saveClaimedTasks(claimed);
                saveLocalData();
                updateUI();
                showToast(`🏆 +${data.reward} TON! ${task.getName()}`, false);
                try { if (tg && tg.HapticFeedback) tg.HapticFeedback.notificationOccurred('success'); } catch(e) {}
            } else {
                showToast("❌ " + (data.error || "Claim failed"), true);
            }
        } catch (e) {
            showToast("Claim failed, try again", true);
        }
    } else {
        // No wallet — local only (limited value)
        claimed.push(taskId);
        saveClaimedTasks(claimed);

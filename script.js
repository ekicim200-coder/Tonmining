// --- IMPORT ---
import { saveUserToFire, getUserFromFire, initAuth, saveWithdrawalRequest, getHistoryFromFire, saveReferralCode, registerReferral, addReferralCommission, getReferralStats, getKYCStatus, submitKYCRequest } from './firebase-config.js';
...
async function loadServerData(walletAddress) {
    ...
    if (serverData) {
        ...
        // KYC durumunu yükle
        await loadKYCStatus(walletAddress);
...
async function createStarsInvoice(machineId, starAmount) {
    ...
    console.log('✅ Stars invoice created successfully');
...
// KYC durumunu Firebase'den yükle
async function loadKYCStatus(walletAddress) {
    if (!walletAddress) return;
    
    const kycData = await getKYCStatus(walletAddress);
    state.kycStatus = kycData.status;
    state.kycVerified = kycData.verified;
    
    console.log('📋 KYC Status:', state.kycStatus, 'Verified:', state.kycVerified);
}

// KYC UI'yi güncelle
function updateKYCUI() {
    const kycBadge = document.getElementById('kyc-status-badge');
    const kycButton = document.getElementById('kyc-verify-btn');
    
    if (!kycBadge || !kycButton) return;
    
    // Status badge güncelle
    if (state.kycStatus === 'approved') {
        kycBadge.innerHTML = '<i class="fas fa-check-circle"></i> KYC Verified';
        kycBadge.style.background = 'linear-gradient(135deg, rgba(16, 185, 129, 0.2), rgba(5, 150, 105, 0.1))';
        kycBadge.style.border = '1.5px solid #10b981';
        kycBadge.style.color = '#10b981';
        kycBadge.style.display = 'block';
        
        if (kycButton) kycButton.style.display = 'none';
    } else if (state.kycStatus === 'pending') {
        kycBadge.innerHTML = '<i class="fas fa-clock"></i> KYC Pending';
        kycBadge.style.background = 'linear-gradient(135deg, rgba(245, 158, 11, 0.2), rgba(217, 119, 6, 0.1))';
        kycBadge.style.border = '1.5px solid #f59e0b';
        kycBadge.style.color = '#f59e0b';
        kycBadge.style.display = 'block';
        
        if (kycButton) kycButton.style.display = 'none';
    } else if (state.kycStatus === 'rejected') {
        kycBadge.innerHTML = '<i class="fas fa-times-circle"></i> KYC Rejected';
        kycBadge.style.background = 'linear-gradient(135deg, rgba(239, 68, 68, 0.2), rgba(220, 38, 38, 0.1))';
        kycBadge.style.border = '1.5px solid #ef4444';
...
// KYC bölümünü göster (çekim sonrası)
function showKYCSection(amount) {
    const kycSection = document.getElementById('kyc-section');
    const kycAmountDisplay = document.getElementById('kyc-ton-amount');
    
    if (!kycSection) {
        console.error('❌ KYC section bulunamadı!');
        return;
    }
    
    // Miktarı göster
    if (kycAmountDisplay) {
        kycAmountDisplay.textContent = amount.toFixed(2);
    }
    
    // KYC durumuna göre mesaj göster
    const kycMessage = document.getElementById('kyc-message');
    if (kycMessage) {
        if (state.kycStatus === 'approved') {
            kycMessage.innerHTML = '<i class="fas fa-check-circle" style="color: #10b981;"></i> Your KYC is verified. Withdrawal will be processed.';
        } else if (state.kycStatus === 'pending') {
            kycMessage.innerHTML = '<i class="fas fa-clock" style="color: #f59e0b;"></i> Your KYC is pending review. Withdrawal on hold.';
        } else {
            kycMessage.innerHTML = '<i class="fas fa-shield-alt" style="color: #f59e0b;"></i> KYC verification required for withdrawals.';
        }
    }
    ...
    // Smooth scroll
    setTimeout(() => {
        kycSection.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }, 300);
    
    console.log('✅ KYC section gösterildi. Miktar:', amount, 'Status:', state.kycStatus);
}

// KYC başvurusu yap
window.startKYCVerification = async function() {
    if (!state.wallet) {
    ...
    if (state.kycStatus === 'pending') {
        showToast("KYC already pending!", true);
        return;
    }
    
    if (state.kycStatus === 'approved') {
        showToast("Already verified!", false);
        return;
    }
    
    showToast("Starting KYC...", false);
    
    // KYC URL'i - gerçek KYC servisi buraya entegre edilecek
    const kycUrl = "https://sumsub.com"; // ⚠️ GERÇEK KYC URL'İNİZİ BURAYA EKLEYIN
    
    // Firebase'e KYC başvurusunu kaydet
    const kycData = {
        kycUrl: kycUrl,
        email: null,
        name: null
    ...
    const success = await submitKYCRequest(state.wallet, kycData);
    
    if (success) {
        state.kycStatus = 'pending';
        updateKYCUI();
        showToast("✅ KYC submitted!", false);
        ...
        showToast("Failed to submit KYC!", true);
    }
}
        }
    } catch (error) {

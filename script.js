...
let state = { 
    ...
    tonToEur: 2.5, // TON/EUR kuru (varsayılan)
    lastPriceUpdate: 0
};
...
function init() {
    ...
    setupTonConnect();
    initAdsgram();
    updateTonPrice(); // İlk kur güncellemesi
    ...
    setInterval(updateTonPrice, 300000); // Her 5 dakikada bir kur güncelle
...
function updateUI() {
    const balanceElement = document.getElementById('totalBal');
    if (balanceElement) {
        const eurValue = (state.balance * state.tonToEur).toFixed(2);
        balanceElement.innerHTML = state.balance.toFixed(2);
        
        // EUR değerini göster
        const balBox = document.querySelector('.bal-val');
        if (balBox) {
            balBox.innerHTML = `<span id="totalBal">${state.balance.toFixed(2)}</span> <span class="bal-unit">TON</span> <span style="font-size: 0.7rem; color: #888; margin-left: 8px;">(≈€${eurValue})</span>`;
        }
    }
    ...
    const dailyEur = (daily * state.tonToEur).toFixed(2);
    document.getElementById('d-daily').innerHTML = `${daily} <span style="font-size: 0.7rem; color: #888;">(≈€${dailyEur})</span>`;
}
...
async function createStarsInvoice(machineId, starAmount) {
    ...
    console.log('📤 Creating invoice - Machine:', machineId, 'User:', userId);
...
// KYC doğrulama sayfasını açma
window.openKYC = function() {
    showToast("Opening KYC verification...", false);
    
    // KYC sağlayıcı URL'i - buraya gerçek KYC servisi eklenecek
...
// --- TON/EUR PRICE UPDATE ---
async function updateTonPrice() {
    try {
        // CoinGecko Free API kullanarak TON/EUR kurunu çekiyoruz
        const response = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=the-open-network&vs_currencies=eur');
        const data = await response.json();
        
        if (data && data['the-open-network'] && data['the-open-network'].eur) {
            const newRate = data['the-open-network'].eur;
            state.tonToEur = newRate;
            state.lastPriceUpdate = Date.now();
            console.log(`✅ TON/EUR kuru güncellendi: €${newRate}`);
            updateUI(); // UI'yi yeni kurla güncelle
            saveLocalData(); // Kuru kaydet
        }
    } catch (error) {
        console.error('❌ Fiyat güncellenemedi:', error);
        // Hata durumunda varsayılan değer kullanılır (state.tonToEur = 2.5)
    }
}

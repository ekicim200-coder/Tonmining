// ==========================================
// 1. FIREBASE BAĞLANTI AYARLARI (EN ÜSTE)
// ==========================================
...
// Firebase'i "Compat" modunda başlatıyoruz (Görselleri bozmaz)
...
renderMachines();
// --- SCRIPT.JS EN ALTINA YAPIŞTIRILACAK GÜVENLİ KOD ---
...
firebase.auth().onAuthStateChanged(async (user) => {
    if (user) {
        ...
        if (docSnap.exists()) {
            // Sunucudaki parayı senin 'minedBTC' değişkenine yükler
            minedBTC = docSnap.data().balance || 0;
            const ownedList = docSnap.data().ownedMachines || [1];
            machines.forEach(m => { m.owned = ownedList.includes(m.id); });
        } else {
            // Yeni kullanıcıysa sıfır bakiye ile başlat
            await userDocRef.set({ balance: 0, ownedMachines: [1], createdAt: new Date() });
        }
        renderMachines(); // Veriler yüklenince ekranı tazele
    } else {
        firebase.auth().signInAnonymously();
    }
});

// Yedekleme Motoru: 30 saniyede bir sessizce buluta kaydeder
setInterval(() => {
    if (userDocRef) {
        userDocRef.update({ balance: minedBTC });
        document.getElementById('sync-status').innerText = "Bulut güncellendi: " + new Date().toLocaleTimeString();
    }
}, 30000);
// ==========================================
// SON ADIM: BULUT HAFIZA MOTORU
// ==========================================

// 1. KULLANICIYI TANI VE VERİLERİ YÜKLE
auth.onAuthStateChanged(async (user) => {
    if (user) {
        // Kullanıcı bulundu, buluttaki çekmecesini tanımla
        userDocRef = db.collection("users").doc(user.uid);
        
        // Veritabanından eski verileri çek
        const docSnap = await userDocRef.get();
        if (docSnap.exists()) {
            // VARSA: Eski bakiyeyi ve makineleri yükle
            minedBTC = docSnap.data().balance || 0;
            const ownedList = docSnap.data().ownedMachines || [1];
            
            // Makinelerin "sahip olma" durumunu güncelle
            machines.forEach(m => { 
                if(ownedList.includes(m.id)) m.owned = true; 
            });
            
            addLog("Cloud storage connected. Data loaded.", "success");
        } else {
            // YOKSA: Yeni kullanıcı için başlangıç dosyası oluştur
            await userDocRef.set({ 
                balance: 0, 
                ownedMachines: [1], 
                createdAt: new Date() 
            });
            addLog("New miner profile created.", "info");
        }
        
        // Veriler yüklenince ekranı tazele (Makineler "ONLINE" olsun)
        renderMachines();
    } else {
        // Kimse yoksa sessizce giriş yap
        auth.signInAnonymously();
    }
});

// 2. YAVAŞ YEDEKLEME (Optimizasyon)
// Sunucuyu yormamak için her 30 saniyede bir bakiyeyi buluta kaydeder
setInterval(() => {
    if (userDocRef) {
        userDocRef.update({ balance: minedBTC });
        // Ekranda küçük bir durum yazısı varsa güncelle
        const syncLabel = document.getElementById('sync-status');
        if(syncLabel) syncLabel.innerText = "Bulut güncellendi: " + new Date().toLocaleTimeString();
    }
}, 30000);// ==========================================
// SON ADIM: BULUT HAFIZA MOTORU
// ==========================================

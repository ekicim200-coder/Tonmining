...
function setupEventListeners() {
    ...
    if (adBtn) {
        ...
        console.log("✅ Ad button listener added");
    }
    ...
    if (copyBtn) {
        ...
        console.log("✅ Copy ref button listener added");
    }
    
    const shareBtn = document.getElementById('share-ref-btn');
    if (shareBtn) {
        shareBtn.addEventListener('click', () => shareReferralLink());
        console.log("✅ Share ref button listener added");
    }
    
    // Navigation - Her butonu tek tek kontrol et
    ...
    navItems.forEach((item, index) => {
        ...
        // Son item (info butonu) onclick ile ayrı çalışıyor, onu atla
        if (index < views.length) {
            const viewName = views[index];
            console.log(`➕ Adding listener for view: ${viewName}`);
            
            item.addEventListener('click', function(e) {
                console.log(`🖱️ Clicked nav item ${index} (${viewName})`);
                e.preventDefault();
                e.stopPropagation();
                go(viewName, this);
            });
...
function grantReferralBonus() {
    ...
    updateUI();
    drawChart();
    renderInv(); // Inventory'yi yenile
    
    console.log("🎁 Bonus Machine Granted: Basic Chip v1!");
    showToast("🎁 Bonus Machine Unlocked!", false);
}
...
function checkFree() {
    ...
    if (state.freeEnd > 0 && Date.now() < state.freeEnd) {
        // FREE makine aktif, timer göster
        ...
        // Timer bitti veya hiç aktif değil
        if (state.freeEnd > 0) {
            // FREE makine süresi bitti, kaldır
            ...
            if (freeMachine) {
                ...
                // FREE timer'ı sıfırla
                state.freeEnd = 0;
                
                console.log(`⏱️ FREE makine süresi bitti, ${freeMachine.rate} GH/s çıkarıldı`);
...
async function grantMachine(mid) {
    ...
    if (mid === 999) {
        ...
        state.inv.push({ mid, uid: Date.now(), bonus: true, isFree: true }); // isFree flag eklendi
        state.hashrate += m.rate; // FREE makine için de hashrate ekle
    } else {
        state.inv.push({ mid, uid: Date.now(), bonus: false });
        state.hashrate += m.rate; // Normal makineler için hashrate
        
        // ✅ REFERRAL COMMISSION EKLE
        if (state.wallet && m.price > 0) {
            try {
                const commissionAdded = await addReferralCommission(state.wallet, m.price);
                if (commissionAdded) {
                    console.log(`✅ Referral commission added for ${m.price} TON purchase`);
                    
                    // Referral history'yi yenile
                    if (document.getElementById('v-referral').classList.contains('active')) {
                        await renderReferralHistory();
                    }
                }
            } catch (error) {
...
// --- NAV ---
function go(id, el) {
    console.log(`📍 go() çağrıldı: ${id}`);
    
    document.querySelectorAll('.view').forEach(x => x.classList.remove('active'));
    const targetView = document.getElementById('v-'+id);
    
    if (!targetView) {
        console.error(`❌ View bulunamadı: v-${id}`);
        return;
    }
...
// Global fonksiyon - HTML'den direkt çağrılabilir
window.goToView = function(id, el) {
    go(id, el);
}
...
function renderInv() {
    ...
    state.inv.slice().reverse().forEach(i => {
        ...
        if (!m) return; // Machine bulunamazsa skip
        
        let isFree = m.id===999;
        let isBonus = i.bonus === true;
        
        let badge = isFree ? 'FREE' : (isBonus ? '🎁 BONUS' : 'ACTIVE');
        let badgeColor = isFree ? 'var(--danger)' : (isBonus ? '#FFD700' : 'var(--success)');
        let badgeBg = isFree ? 'rgba(239, 68, 68, 0.1)' : (isBonus ? 'rgba(255, 215, 0, 0.2)' : 'rgba(16, 185, 129, 0.1)');
        ...
        <div class="card-item">
            ...
            <div class="ci-info">
                <h4>${m.name}</h4>
                <p>⚡ ${m.rate} GH/s ${isFree ? '• 30 min' : ''}</p>
            </div>
            <div class="ci-action">
                <div style="background:${badgeBg}; color:${badgeColor}; padding: 6px 12px; border-radius: 8px; font-weight:bold; font-size:0.75rem; border: 1px solid ${badgeColor};">
                    ${badge}
                </div>
            </div>
        </div>`;
...
// DOM hazır olduğunda init çağır
